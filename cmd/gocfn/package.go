...
var (
	...
	packageOutputTemplateFile = packageCommand.Flag("output-template-file", "The path to the file where the command writes the output AWS CloudFormation template.").Required().ExistingFile()
...
func (c *Cfn) packaage(packageParams *command.PackageParams) {
	c.pckgr.Export(packageParams.TemplateFile)
}
